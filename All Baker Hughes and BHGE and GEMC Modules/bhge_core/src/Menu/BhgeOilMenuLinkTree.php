...
namespace Drupal\bhge_core\Menu;
...
/**
 * Return customized renderable array of menu links.
 */
class BhgeOilMenuLinkTree extends MenuLinkTree {
  ...
  protected function buildItems(array $tree, CacheableMetadata &$tree_access_cacheability, CacheableMetadata &$tree_link_cacheability) {
    ...
    foreach ($tree as $data) {
      ...
      $uuid = substr($link->getPluginId(), strpos($link->getPluginId(), ":") + 1);
      $menu_content = current(\Drupal::entityTypeManager()
        ->getStorage('menu_link_content')
        ->loadByProperties(['uuid' => $uuid]));

      if ($menu_content) {
        $element['description'] = $menu_content->get('description')->value;
        if (strpos($menu_content->get('link')[0]->get('uri')->getCastedValue(), 'entity:node') !== FALSE) {
          $link_nid = Url::fromUri($menu_content->get('link')[0]->get('uri')->getCastedValue())->getRouteParameters();
          if (isset($link_nid) && is_numeric(reset($link_nid))) {
            $node = Node::load(reset($link_nid));
            $element['theme'] = $node->field_color_theme[0] != NULL ? $node->get('field_color_theme')[0]->value : '';
          }
        }
        $element['open_in_new_tab'] = $menu_content->get('open_in_new_tab')->value;
      }
